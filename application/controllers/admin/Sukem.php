...
class Sukem extends CI_Controller {
    ...
    public function index() {
        ...
        if ($logged_in != TRUE || empty($logged_in)) {
            ...
            $data['sukem'] = $this->M_all->get_all_sukem(); 
            ...
            $this->load->view('admin/sukem', $data); 
            $this->load->view('admin/layouts/footer');
        }
    }
    ...
    public function add() {
        ...
        $this->form_validation->set_rules('tgl_kedukaan', 'Tanggal Kedukaan', 'required', [
            'required' => 'Tanggal Kedukaan wajib diisi.'
        ]);
        $this->form_validation->set_rules('lokasi_kedukaan', 'Lokasi Kedukaan', 'required', [
            'required' => 'Lokasi Kedukaan wajib diisi.'
        ]);
        ...
        $this->form_validation->set_rules('hubungan', 'Hubungan', 'required', [
            'required' => 'Hubungan dengan Almarhum/Almarhumah wajib diisi.'
        ]);
    
        if ($this->form_validation->run() == FALSE) {
            $this->session->set_flashdata('error', validation_errors());
            redirect('admin/sukem');
        } else {
            // **Generate Nomor Pengajuan**
            $date = date('dmy'); // Format tanggal ddmmyy
            $random_number = mt_rand(1000000, 9999999); // Angka random 7 digit
            $no_pengajuan = 'PK' . $date . $random_number; // Format: PDddmmyyXXXXXXX
            ...
            $data = [
                ...
                'tgl_kedukaan' => $this->input->post('tgl_kedukaan'),
                'lokasi_kedukaan' => $this->input->post('lokasi_kedukaan'),
            ...
            // **Insert ke Database**
            if ($this->M_all->insert_suratsukem($data)) {
                $this->session->set_flashdata('success', 'Pengajuan Surat Sukem berhasil ditambahkan. No Pengajuan: ' . $no_pengajuan);
            } else {
                $this->session->set_flashdata('error', 'Gagal menambahkan pengajuan Surat Sukem.');
            }
    ...
    public function update() {
        ...
        $tgl_kedukaan = $this->input->post('tgl_kedukaan');
        $lokasi_kedukaan = trim($this->input->post('lokasi_kedukaan'));
        $keterangan = trim($this->input->post('keterangan'));
        $nama_pelapor = trim($this->input->post('nama_pelapor'));
        $hubungan = trim($this->input->post('hubungan'));
        $tanda_tangan = $this->input->post('tanda_tangan');
        $status = $this->input->post('status');
        $tgl_update = date('Y-m-d H:i:s'); // Tanggal update otomatis
    
        // Validasi data wajib
        if (!$id || !$no_surat || !$nik || !$tgl_kedukaan || !$lokasi_kedukaan || !$nama_pelapor || !$hubungan || !$tanda_tangan) {
            $this->session->set_flashdata('error', 'Data tidak valid! Pastikan semua data wajib diisi.');
            redirect('admin/sukem');
        }
    
        // Cek apakah data ada di database
        $data_lama = $this->db->get_where('tb_surat_sukem', ['id' => $id])->row();
        if (!$data_lama) {
        ...
        $data = [
            'no_surat' => $no_surat,
            'nik' => $nik,
            'tgl_kedukaan' => $tgl_kedukaan,
            'lokasi_kedukaan' => $lokasi_kedukaan,
        ...
        $update = $this->M_all->update_sukem($id, $data);
    
        if ($update) {
            $this->session->set_flashdata('success', 'Data surat sukem berhasil diperbarui.');
        } else {
            $this->session->set_flashdata('error', 'Gagal memperbarui data surat sukem.');
        }
...
public function delete($id) {
    ...
    $sukem = $this->M_all->get_sukem_by_id($id);

    // Cek apakah data surat lahir ditemukan
    if (!$sukem) {
        $this->session->set_flashdata('error', 'Data surat sukem tidak ditemukan.');
        redirect('admin/sukem');
    }

    // Delete the data
    if ($this->M_all->delete_sukem($id)) {
        $this->session->set_flashdata('success', 'Surat sukem berhasil dihapus.');
    } else {
        $this->session->set_flashdata('error', 'Gagal menghapus surat sukem.');
    }
...
public function downloadskk($encoded_alias = null) {
    ...
    // Ambil data dari tb_surat_sukem
    $data['item'] = $this->Model_warga->get_skk_by_alias($id_alias)->row();
    ...
    $pdf->SetTitle('Surat Keterangan sukem');
    $pdf->SetSubject('Surat Keterangan sukem');

    // Atur Logo di Kiri
$pdf->Image(base_url('assets/aplikasi/Kabupaten_Sukabumi.png'), 10, 25, 20); // (file, X, Y, Width)

    // Load View dan Generate PDF
    $html = $this->load->view('admin/surat/skk', $data, true);
    $pdf->writeHTML($html, true, false, true, false, '');

    $pdf->Output('surat_keterangan_sukem.pdf', 'I');
}
    ...
    public function kirim_surat($encoded_alias) {
        ...
        // Ambil data surat sukem berdasarkan id_alias
        $pengajuan = $this->Model_warga->get_skk_by_alias($id_alias)->row();
