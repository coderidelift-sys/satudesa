...
class Bape extends CI_Controller {
    ...
    public function index() {
        ...
        if ($logged_in != TRUE || empty($logged_in)) {
            ...
            $data['bape'] = $this->M_all->get_all_bape(); 
            ...
            $this->load->view('admin/bape', $data); 
            $this->load->view('admin/layouts/footer');
        }
    }
    ...
    public function add() {
        ...
        $this->form_validation->set_rules('daftar_saksi', 'Daftar Saksi', 'required', [
            'required' => 'Daftar Saksi wajib diisi.'
        ]);
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $no_pengajuan = 'PBAP' . $date . $random_number; // Format: PDddmmyyXXXXXXX
            ...
            $data = [
                ...
                'lokasi_pemakaman' => $this->input->post('lokasi_pemakaman'),
                'jam' => $this->input->post('jam'),
                'keterangan' => $this->input->post('keterangan'),
                'daftar_saksi' => $this->input->post('daftar_saksi'),
            ...
            // **Insert ke Database**
            if ($this->M_all->insert_bape($data)) {
                $this->session->set_flashdata('success', 'Pengajuan Surat Berita Acara Penguburan berhasil ditambahkan. No Pengajuan: ' . $no_pengajuan);
                ...
                if ($user_id && $username) {
                    ...
                    $this->M_log->log_activity($user_id, $username, 'BA Penguburan', 'Menambahkan  data pengajuan surat BA Penguburan');
                }
            } else {
                $this->session->set_flashdata('error', 'Gagal menambahkan pengajuan Surat Berita Acara Penguburan.');
            }
    ...
    public function update() {
        ...
        $data_lama = $this->db->get_where('tb_ba_penguburan', ['id' => $id])->row();
        if (!$data_lama) {
        ...
        $data = [
            ...
            'status' => $status,
            'lokasi_pemakaman' => $lokasi_pemakaman,
            'jam' => $jam,
            'no_wa' => $no_wa,
        ...
        $update = $this->M_all->update_bape($id, $data);
        ...
        if ($update) {
            ...
            $user_id = $this->session->userdata('user_id');
                ...
                if ($user_id && $username) {
                    // Simpan log aktivitas logout
                    $this->load->model('M_log');
                    $this->M_log->log_activity($user_id, $username, 'BA Penguburan', 'Mengupdate data pengajuan surat BA Penguburan');
                }
        } else {
...
public function delete($id) {
    ...
    $bape = $this->M_all->get_bape_by_id($id);

    // Cek apakah data surat lahir ditemukan
    if (!$bape) {
        $this->session->set_flashdata('error', 'Data berita acara tidak ditemukan.');
        redirect('admin/bape');
    }

    // Delete the data
    if ($this->M_all->delete_bape($id)) {
        ...
        $user_id = $this->session->userdata('user_id');
                ...
                if ($user_id && $username) {
                    // Simpan log aktivitas logout
                    $this->load->model('M_log');
                    $this->M_log->log_activity($user_id, $username, 'BA Penguburan', 'Menghapus  data pengajuan surat BA Penguburan');
                }
    } else {
        $this->session->set_flashdata('error', 'Gagal menghapus berita acara.');
    }

    redirect('admin/bape');
}

public function downloadsbap($encoded_alias = null) {
    ...
    // Ambil data dari tb_surat_Berita Acara Penguburan
    $data['item'] = $this->Model_warga->get_sbap_by_alias($id_alias)->row();
    ...
    $pdf->SetTitle('Surat Keterangan BA Penguburan');
    $pdf->SetSubject('Surat Keterangan BA Penguburan');

    // Atur Logo di Kiri
$pdf->Image(base_url('assets/aplikasi/Kabupaten_Sukabumi.png'), 10, 25, 20); // (file, X, Y, Width)

    // Load View dan Generate PDF
    $html = $this->load->view('admin/surat/sbap', $data, true);
    $pdf->writeHTML($html, true, false, true, false, '');

    $pdf->Output('surat_keterangan_BA_Penguburan.pdf', 'I');
}
    ...
    public function kirim_surat($encoded_alias) {
        ...
        // Ambil data surat Berita Acara Penguburan berdasarkan id_alias
        $pengajuan = $this->Model_warga->get_sbap_by_alias($id_alias)->row();
        ...
        if ($response) {
            ...
            $this->session->set_flashdata('success', 'Pesan berhasil dikirim!');
                ...
                if ($user_id && $username) {
                    // Simpan log aktivitas logout
                    $this->load->model('M_log');
                    $this->M_log->log_activity($user_id, $username, 'BA Penguburan', 'Mengirimkan file PDF pengajuan surat BA Penguburan');
                }
        }
