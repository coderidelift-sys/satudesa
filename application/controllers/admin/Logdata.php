...
class Logdata extends CI_Controller {
    ...
    public function index() {
        ...
        if ($logged_in != TRUE || empty($logged_in)) {
            ...
            $this->load->view('admin/logdata'); 
            $this->load->view('admin/layouts/footer');
        }
    }
    
    // Server-side processing for datatable
    public function get_logdata_json() {
        $this->load->model('M_all');
        ...
        $data = $this->M_all->get_logdata_server_side($start, $length, $search, $columnName, $columnSortOrder);
        
        $output = array(
            "draw" => $draw,
            "recordsTotal" => $this->M_all->count_all_logdata(),
            "recordsFiltered" => $this->M_all->count_filtered_logdata($search),
            "data" => $data
        );
