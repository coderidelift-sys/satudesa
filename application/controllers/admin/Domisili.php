...
class Domisili extends CI_Controller {
    ...
    $data['pending_notifications'] = $this->M_all->get_all_pending_notifications();
        ...
        if ($logged_in != TRUE || empty($logged_in)) {
            ...
            $data['domisili'] = $this->M_all->get_all_domisili(); 
            ...
            $this->load->view('admin/suratdomisili', $data); 
            $this->load->view('admin/layouts/footer');
        }
    }
    ...
    public function add() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $no_pengajuan = 'PD' . $date . $random_number; // Format: PDddmmyyXXXXXXX
            ...
            // **Insert ke Database**
            if ($this->M_all->insert_suratdomisili($data)) {
                $this->session->set_flashdata('success', 'Pengajuan Surat Domisili berhasil ditambahkan. No Pengajuan: ' . $no_pengajuan);
                ...
                if ($user_id && $username) {
                    ...
                    $this->M_log->log_activity($user_id, $username, 'Domisili', 'Menambahkan data pengajuan surat Domisili');
                }
            } else {
                $this->session->set_flashdata('error', 'Gagal menambahkan pengajuan Surat Domisili.');
            }
    ...
    public function update() {
        ...
        $data_lama = $this->db->get_where('tb_surat_domisili', ['id' => $id])->row();
        if (!$data_lama) {
        ...
        $update = $this->M_all->update_domisili($id, $data);
    
        if ($update) {
            $this->session->set_flashdata('success', 'Data surat domisili berhasil diperbarui.');
            $user_id = $this->session->userdata('user_id');
                $username = $this->session->userdata('username');
                ...
                if ($user_id && $username) {
                    // Simpan log aktivitas logout
                    $this->load->model('M_log');
                    $this->M_log->log_activity($user_id, $username, 'Domisili', 'Mengupdate data pengajuan surat Domisili');
                }
        } else {
            $this->session->set_flashdata('error', 'Gagal memperbarui data surat domisili.');
        }
...
public function delete($id) {
    ...
    $domisili = $this->M_all->get_domisili_by_id($id);

    // Cek apakah data surat lahir ditemukan
    if (!$domisili) {
        $this->session->set_flashdata('error', 'Data surat domisili tidak ditemukan.');
        redirect('admin/domisili');
    }

    // Delete the data
    if ($this->M_all->delete_domisili($id)) {
        $this->session->set_flashdata('success', 'Surat domisili berhasil dihapus.');
        $user_id = $this->session->userdata('user_id');
                $username = $this->session->userdata('username');
                ...
                if ($user_id && $username) {
                    // Simpan log aktivitas logout
                    $this->load->model('M_log');
                    $this->M_log->log_activity($user_id, $username, 'Domisili', 'Menghapus data pengajuan surat Domisili');
                }
    } else {
        $this->session->set_flashdata('error', 'Gagal menghapus surat domisili.');
    }
...
public function downloadsd($encoded_alias = null) {
    ...
    // Ambil data dari tb_surat_domisili
    $data['item'] = $this->Model_warga->get_skd_by_alias($id_alias)->row();
    ...
    $pdf->SetTitle('Surat Keterangan Domisili');
...
$pdf->Image(base_url('assets/aplikasi/Kabupaten_Sukabumi.png'), 10, 25, 20); // (file, X, Y, Width)
    ...
    $html = $this->load->view('admin/surat/skd', $data, true);
    $pdf->writeHTML($html, true, false, true, false, '');

    $pdf->Output('surat_keterangan_domisili.pdf', 'I');
}
    ...
    public function kirim_surat($encoded_alias) {
        ...
        // Ambil data surat domisili berdasarkan id_alias
        $pengajuan = $this->Model_warga->get_skd_by_alias($id_alias)->row();
        ...
        if ($response) {
            ...
            $username = $this->session->userdata('username');
                ...
                if ($user_id && $username) {
                    // Simpan log aktivitas logout
                    $this->load->model('M_log');
                    $this->M_log->log_activity($user_id, $username, 'Domisili', 'Mengirimkan file PDF pengajuan surat Domisili');
                }
        }
